...
class RedisClient {
   ...
   */
  connectHandler() {
    this.logger.info(`Connected to redis ${this.config.uri}`);
  }
  ...
  /**
   * Scan a zset under the given key for values matching the specified pattern
   * @param {string} key
   * @param {string} pattern
   ...
   */
  async zscan(key, pattern, count = null, json = true) {
    const results = [];
    let args = [key, '0', 'MATCH', pattern];
    if (count) args.push(...['COUNT', count]);

    // push results from a batch into results array
    const pushResults = (batch, results) => {
      for (let i = 0; i < batch.length; i += 2) {
        results.push(json ? JSON.parse(batch[i]) : batch[i]);
      }
    };

    // fetch first batch
    let [cursor, batch] = await this.redis.zscan(...args);
    pushResults(batch, results);
    ...
    while (cursor !== '0') {
      ...
      args[1] = cursor;
      [cursor, batch] = await this.redis.zscan(...args);
      pushResults(batch, results);
    }
    return results;
  }
